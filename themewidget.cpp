...
QChart *ThemeWidget::createPieChart() const
{
    ...
    chart->setTitle("COLLECTIONS VS DONATIONS MONEY");
    ...
    queryDonations.next();
    queryCollections.next();

    double amountSum = queryDonations.value(0).toDouble();
    double collectionSum = queryCollections.value(0).toDouble();

    // Add the sum of budgets and collections to the pie chart
    series->append("Budget", amountSum);
    ...
    QPieSlice *budgetSlice = series->slices().at(0);
        budgetSlice->setLabel(QString("Budget: %1").arg(amountSum));
        budgetSlice->setLabelPosition(QPieSlice::LabelInsideNormal);

        QPieSlice *collectionSlice = series->slices().at(1);
        collectionSlice->setLabel(QString("Collection: %1").arg(collectionSum));
