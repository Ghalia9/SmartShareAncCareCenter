...
DataTable ThemeWidget::generateRandomData(int valueMax, int valueCount) const
{
    DataTable dataTable;
...
QChart *ThemeWidget::createPieChart() const
{
    ...
    chart->setTitle("BUDGET VS Donquantity");

    qreal pieSize = 10.0;
    QPieSeries *series = new QPieSeries(chart);

    // Get the sum of budgets and Donquantity from the Collects table
    QSqlQuery query("SELECT SUM(budget_S), SUM(Donquantity) FROM Services");
    query.next();
    double budgetSum = query.value(0).toDouble();
    double DonquantitySum = query.value(1).toDouble();

    // Add the sum of budgets and collections to the pie chart
    series->append("Budget", budgetSum);
    series->append("Donquantity", DonquantitySum);
    ...
    QPieSlice *budgetSlice = series->slices().at(0);
        budgetSlice->setLabel(QString("Budget: %1").arg(budgetSum));
        budgetSlice->setLabelPosition(QPieSlice::LabelInsideNormal);

        QPieSlice *DonquantitySlice = series->slices().at(1);
        DonquantitySlice->setLabel(QString("Collection: %1").arg(DonquantitySum));
        DonquantitySlice->setLabelPosition(QPieSlice::LabelInsideNormal);
