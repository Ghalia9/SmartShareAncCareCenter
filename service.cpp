...
QSqlQueryModel * Service::afficher()
{
    ...
    model->setHeaderData(1,Qt::Horizontal,QObject::tr("serviceName"));
    model->setHeaderData(2,Qt::Horizontal,QObject::tr("Dontype"));
...
{
    QSqlQuery query;
        query.prepare("UPDATE SERVICES SET IDservice = :IDservice, serviceName = :serviceName, budget_S = :budget_S, Dontype = :Dontype, Donquantity = :Donquantity WHERE IDservice LIKE '"+IDservice+"%'");
        query.bindValue(":IDservice", IDservice);
        query.bindValue(":serviceName", serviceName);
        query.bindValue(":budget_S", budget_S);
        query.bindValue(":Dontype", Dontype);
